...
const emptyMessageErorr = document.getElementById("title-empty-error");
const formatMessageErorr = document.getElementById("title-format-error");
const errorMessageInput = document.getElementById("date-error");
...
function showBadgeState(date, completed) {
    if (completed) {
        ...
        const today = parseInt(new Date().toISOString().slice(0, 10).split("-").join(""));
        const dueDate = parseInt(new Date(date).toISOString().slice(0, 10).split("-").join(""));
        const daysNumber = dueDate - today;
        if (daysNumber === 0) {
            return `<span
...
function changeColumn(from, to, taskId) {
...
function deleteTask(from, taskId) {
    tasks[from] = tasks[from].filter((task) => task.id !== taskId);
    save();
    renderTasks();
    showNotification("Task deleted successfully", "success");
}
function editTask(from, taskId) {
...
function validationDate() {
    ...
    if (dueDate < today) {
        ...
        date.classList.add("focus:border-red-500", "focus:ring-red-500", "border-red-500");
        date.classList.remove("focus:border-indigo-500", "focus:ring-indigo-500", "border-slate-300");
        return false;
    }
    else {
        removeErorr();
...
function validationTitle() {
    ...
    if (!titleInput) {
        ...
        title.classList.add("focus:border-red-500", "focus:ring-red-500", "border-red-500");
        title.classList.remove("focus:border-indigo-500", "focus:ring-indigo-500", "border-slate-300");
        return false;
    }
...
function removeErorr() {
    emptyMessageErorr.classList.add("hidden");
    formatMessageErorr.classList.add("hidden");
    title.classList.remove("focus:border-red-500", "focus:ring-red-500", "border-red-500");
    title.classList.add("focus:border-indigo-500", "focus:ring-indigo-500", "border-slate-300");
    errorMessageInput.classList.add("hidden");
    date.classList.remove("focus:border-red-500", "focus:ring-red-500", "border-red-500");
    date.classList.add("focus:border-indigo-500", "focus:ring-indigo-500", "border-slate-300");
}
function showNotification(t, e) {
